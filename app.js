...
app.post('/books', (req, res) => {
    ...
    db.collection('books').insertOne(book)
        .then(result => {
            ...
            res.status(500).json({ err: 'Coudnt not create a new document' })
        })
})
...
app.delete('/books/:id',(req,res)=>{
    if (ObjectId.isValid(req.params.id)) {
        db.collection('books').deleteOne({ _id: new ObjectId(req.params.id) }).then((result) => {
            ...
            res.status(500).json({ error: 'Coudnt Delete the document' })
        })  //cusor toArray forEach
...
if (ObjectId.isValid(req.params.id)) {
    db.collection('books').updateOne({ _id: new ObjectId(req.params.id) },{$set:update}).then((result) => {
        res.status(200).json(result)
    }).catch(() => {
        res.status(500).json({ error: 'Coudnt Update the document' })
    })  //cusor toArray forEach
